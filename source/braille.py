...
def _filterFormatConfig(formatConfig):
	"""
	Returns the filtered version of the provided formatConfig dictionary,
	in which all entries are set to C{False} that aren't currently shown in braille.
	This function should not be used by third party code.
	@param formatConfig: The formatting config.
	@type formatConfig: {str : bool, ...}
	"""
	formatConfig = {
		key: value
		if key not in _IGNORED_FORMAT_CONFIG
		else False
...
class TextInfoRegion(Region):
	...
	def _addTextWithFields(self, info, formatConfig, isSelection=False):
		...
		formatConfig = _filterFormatConfig(formatConfig)
